...
export async function sellGiftCard(data) {
  const response = await api.post(`sell_Gift_Card`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
  return response.data;
};

export async function verification(data) {
  const response = await api.post(`req_verification`, data, { headers: { 'Content-Type': 'multipart/form-data' } });
  return response.data;
};
